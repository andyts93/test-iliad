...
<template>
    <div class="relative inline-block">
        <div class="relative">
            <input type="text" aria-label="Search product" v-model="queryTerm" class="h-10 cst-input" placeholder="Search product" @focus="resultsOpen = true"/>
        ...
        <ul v-if="resultsOpen && products.length > 0" class="absolute right-0 top-10 bg-gray-50 shadow-md min-w-full mt-1 rounded-xl overflow-hidden">
            <li v-for="product in products" :key="product.id">
                <a href="#" class="px-4 py-0.5 flex justify-between gap-8 items-center cursor-pointer hover:bg-gray-300" @click="selectProduct(product)">
